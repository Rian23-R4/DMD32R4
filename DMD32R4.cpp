...
DMD::DMD(byte panelsWide, byte panelsHigh, byte PIN_DMD_nOE, byte PIN_DMD_A, byte PIN_DMD_B, byte PIN_DMD_CLK, byte PIN_DMD_SCLK, byte PIN_DMD_R_DATA)
{
    ...
    vspi->begin(PIN_DMD_CLK, VSPI_MISO, PIN_DMD_R_DATA, PIN_DMD_SCLK);
    ...
    ledcAttachPin(PIN_DMD_nOE, ledChannel);
...
--------------------------------------------------------------------------------------*/
void DMD::scanDisplayBySPI(byte PIN_DMD_nOE, byte PIN_DMD_A, byte PIN_DMD_B, byte PIN_DMD_CLK, byte PIN_DMD_SCLK, byte PIN_DMD_R_DATA, bool stageSPI)
{
    ...
    if (stageSPI)
    {
        ...
        // OE_DMD_ROWS_OFF(PIN_DMD_nOE);
        OE_DMD_ROWS_ON(PIN_DMD_nOE);
        LATCH_DMD_SHIFT_REG_TO_OUTPUT(PIN_DMD_SCLK);
        switch (bDMDByte)
        {
        case 0: // row 1, 5, 9, 13 were clocked out
            LIGHT_DMD_ROW_01_05_09_13(PIN_DMD_B, PIN_DMD_A);
            bDMDByte = 1;
            break;
        case 1: // row 2, 6, 10, 14 were clocked out
            LIGHT_DMD_ROW_02_06_10_14(PIN_DMD_B, PIN_DMD_A);
            bDMDByte = 2;
            break;
        case 2: // row 3, 7, 11, 15 were clocked out
            LIGHT_DMD_ROW_03_07_11_15(PIN_DMD_B, PIN_DMD_A);
            bDMDByte = 3;
            break;
        case 3: // row 4, 8, 12, 16 were clocked out
            LIGHT_DMD_ROW_04_08_12_16(PIN_DMD_B, PIN_DMD_A);
            bDMDByte = 0;
            break;
        }
        // OE_DMD_ROWS_ON(PIN_DMD_nOE);
        ledcWrite(ledChannel, cr);
    }
}
